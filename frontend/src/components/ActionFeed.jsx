...
export default function ActionFeed({ risk, actions: initialActions, livePhase, aiBriefing, cyclone }) {
  ...
  const summary = aiBriefing?.executive_summary || risk?.executive_summary || 
    'Severe cyclonic storm intensifying. Immediate emergency shelter activations and coastal perimeter evacuations recommended.';
  const threatLevel = aiBriefing?.threat_level || risk?.threat_level;
  const evidenceList = aiBriefing?.evidence_used || [];
  const aiMode = aiBriefing?.mode;
